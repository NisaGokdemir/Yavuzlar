<?php
    ...
    $result = getFoodsByCompany($companyId);

    $restaurants = getRestaurantsByCompanyId($companyId);

    if (isset($_POST['search'])) {
        $name = $_POST['name'];
        $result = searchFoodByName($name);
    }
?>
<!DOCTYPE html>
<html lang="en">
...
<nav class="navbar bg-body-tertiary fixed-top">
    ...
    <div class="container" style="margin-top: 100px;">
        ...
        <h5 class="title mb-4">Yemekler</h5>
        <form method="POST" action="">
            <div class="input-group mb-3">
                <input type="text" class="form-control" name="name" placeholder="Yemek ismi girin" required>
                <button class="btn btn-outline-secondary" type="submit" name="search">Ara</button>
            </div>
        </form>
        <?php foreach ($result as $food): ?>
            <div class="input-group mb-3">
                <div class="form-control"><?php echo $food['name']; ?> - <?php echo $food['price']; ?>₺</div>
                <a href="updateFood.php?id=<?php echo $food['id']; ?>" class="btn btn-outline-secondary">Güncelle</a>
                <a href="deleteFood.php?id=<?php echo $food['id']; ?>" class="btn btn-outline-secondary">Sil</a>
            </div>
        <?php endforeach; ?>

        <a href="addRestaurantCupon.php" class="btn btn-primary">Restorana Kupon Ekle</a>

        <form action="addFood.php" method="POST" enctype="multipart/form-data">  
            <h5 class="title mb-4 mt-5">Yemek Ekle</h5>
            <div class="mb-3">  
            ...
            <div class="mb-3">  
                ...
                <input type="text" class="form-control" id="foodDiscount" name="foodDiscount" required>  
            </div>  
            <div class="mb-3">  
