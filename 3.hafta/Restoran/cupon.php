<?php
    ...
    $result = getActiveCupons();

    if (isset($_POST['search'])) {
        $name = $_POST['name'];
        $result = searchCuponByName($name);
    }
?>
<!DOCTYPE html>
<html lang="en">
...
<nav class="navbar bg-body-tertiary fixed-top">
    ...
    <h5 class="title mb-4 mt-5">Kuponlar</h5>
        <form method="POST" action="">
            <div class="input-group mb-3">
                <input type="text" class="form-control" name="name" placeholder="Kupon ismi girin" required>
                <button class="btn btn-outline-secondary" type="submit" name="search">Ara</button>
            </div>
        </form>
        <?php foreach ($result as $cupon): ?>
            <div class="input-group mb-3">
                <div class="form-control"><?php echo $cupon['name']; ?> <?php echo $cupon['discount']; ?></div>
                <a href="updateCupon.php?id=<?php echo $cupon['id']; ?>" class="btn btn-outline-secondary">Güncelle</a>
                <a href="deleteCupon.php?id=<?php echo $cupon['id']; ?>" class="btn btn-outline-secondary">Sil</a>
            </div>
        <?php endforeach; ?>

        <form action="addCupon.php" method="POST" enctype="multipart/form-data">  
            <h5 class="title mb-4 mt-5">Kupon Ekle</h5>
            <div class="mb-3">  
                <label for="companyName" class="form-label">Kupon Adı</label>  
                <input type="text" class="form-control" id="cuponName" name="cuponName" required>  
            </div>  
            <div class="mb-3">  
                <label for="companyName" class="form-label">Kupon Fiyatı</label>  
                <input type="text" class="form-control" id="cuponDiscount" name="cuponDiscount" required>  
            </div>  
